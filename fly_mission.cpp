...
int main(int argc, char** argv)
{
    ...
    try{
        for (YAML::const_iterator it = config["position"].begin(); it != config["position"].end(); ++it) {
	    std::string pos = it->first.as<std::string>();
	    YAML::Node param = it->second;
	    mission_items.push_back(make_mission_item(
                param["latitude_deg"].as<double>(),
                param["longitude_deg"].as<double>(),
                param["relative_altitude_m"].as<float>(),
                param["speed_m_s"].as<float>(),
                param["is_fly_through"].as<bool>(),
                param["gimbal_pitch_deg"].as<float>(),
                param["gimbal_yaw_deg"].as<float>(),
